...
export const ACHIEVEMENTS = [
  ...
  { id: 'vocab-all',        icon: '🎓', title: 'Word Master',       desc: 'Master all 280 SAT vocabulary words' },
  ...
  { id: 'vocab-200',       icon: '🏆', title: 'Vocabulary God',     desc: 'Master all 280 SAT vocabulary words — true word mastery!' },
  ...
  { id: 'vocab-221',     icon: '👑', title: 'Vocab King',         desc: 'Master all 280 SAT vocabulary words — absolute word mastery!' },
...
const CHECKS = {
  ...
  'two-fifty':    (h) => h.reduce((t, s) => t + s.score.total, 0) >= 280,
  ...
  'vocab-all':    () => { try { const v = JSON.parse(localStorage.getItem('sat_prep_vocab') ?? '{}'); return Object.values(v).filter(p => p.mastered).length >= 280 } catch { return false } },
  ...
  'vocab-200':    () => { try { const v = JSON.parse(localStorage.getItem('sat_prep_vocab') ?? '{}'); return Object.values(v).filter(p => p.mastered).length >= 280 } catch { return false } },
  'all-domains':  (h) => {
  ...
  'vocab-221':      () => { try { const v = JSON.parse(localStorage.getItem('sat_prep_vocab') ?? '{}'); return Object.values(v).filter(p => p.mastered).length >= 280 } catch { return false } },
