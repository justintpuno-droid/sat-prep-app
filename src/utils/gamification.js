...
export const ACHIEVEMENTS = [
  ...
  { id: 'vocab-all',        icon: '🎓', title: 'Word Master',       desc: 'Master all 181 SAT vocabulary words' },
  { id: 'formula-10',       icon: '🔢', title: 'Formula Learner',   desc: 'Master 10 math formulas in MathFlash' },
  { id: 'formula-all',      icon: '🧮', title: 'Formula God',       desc: 'Master all 44 math formulas in MathFlash' },
...
const CHECKS = {
  ...
  'vocab-all':    () => { try { const v = JSON.parse(localStorage.getItem('sat_prep_vocab') ?? '{}'); return Object.values(v).filter(p => p.mastered).length >= 181 } catch { return false } },
  ...
  'formula-all':  () => { try { const v = JSON.parse(localStorage.getItem('sat_prep_math_flash') ?? '{}'); return Object.values(v).filter(p => p.mastered).length >= 44 } catch { return false } },
