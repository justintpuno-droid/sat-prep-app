...
import DigitalSATTips from './DigitalSATTips'
import BreathingExercise from './BreathingExercise'
import ScoreCalculator from './ScoreCalculator'
import SATStory from './SATStory'
...
export default function App() {
  ...
  function handlePowerHour() {
    ...
    setSessionConfig({
      mode: 'learning',
      formatLabel: 'Power Hour',
      sessionName: null,
      questions: pool,
      xpMultiplier: 2.0,
      timedChallengeSeconds: 3600,
    })
    setScreen('learning')
  }

  function handleQuickAssessment() {
    const allDomains = [...new Set(allQuestions.map(q => q.domain))]
    const balanced = allDomains.flatMap(domain => {
      const pool = allQuestions.filter(q => q.domain === domain)
      return shuffle(pool).slice(0, 2)
    })
    setSessionConfig({
      mode: 'learning',
      formatLabel: 'Quick Assessment',
      sessionName: null,
      questions: shuffle(balanced).slice(0, 16),
      xpMultiplier: 1.0,
    })
    setScreen('learning')
  }
  ...
  if (screen === 'strategy-guide')
    return <StrategyGuide onBack={() => setScreen('home')} />

  if (screen === 'study-notes')
    return <StudyNotes onBack={() => setScreen('home')} />
  ...
  if (screen === 'digital-sat')
    return <DigitalSATTips onBack={() => setScreen('home')} />

  if (screen === 'breathing')
    return <BreathingExercise onBack={() => setScreen('home')} />

  if (screen === 'score-calculator')
    return <ScoreCalculator onBack={() => setScreen('home')} />

  if (screen === 'sat-story')
    return <SATStory onBack={() => setScreen('home')} />
}
